...
export default function PlanSelection({ language, onSelectPlan, backendUrl, deviceId, currentTier, creditsRemaining, onCancel }: PlanSelectionProps) {
  ...
  async function loadIAPProducts() {
    ...
    try {
      // 1. Primero intentar obtener productos de la tienda nativa (iOS/Android)
      const storeProducts = await BillingService.getStoreProducts();

      if (storeProducts && storeProducts.products && storeProducts.products.length > 0) {
        LoggerService.log('[PlanSelection] 🏪 Usando precios de la tienda nativa');

        // Mapear product IDs a tiers
        ...
        const products = storeProducts.products.map(storeProduct => ({
          product_id: storeProduct.identifier,
          tier: tierMap[storeProduct.identifier] || 'basic',
          name: storeProduct.title,
          price: storeProduct.price,
          priceString: storeProduct.priceString, // Ya incluye símbolo de moneda (ej: "€9.99", "$9.99")
          credits: BillingService.getProductCredits(storeProduct.identifier)
        }));
      ...
      // 2. Fallback: Obtener precios del backend (para web)
      LoggerService.log('[PlanSelection] 🌐 Usando precios del backend (web)');
      ...
      if (pricingData && pricingData.products) {
        const products = pricingData.products.map(product => ({
          ...
          name: product.name,
          price: product.price,
          priceString: product.price.toString(), // Solo el número, sin símbolo
        ...
        const products = [
          { product_id: 'plan_basic', tier: 'basic', name: 'Basic', price: 4.99, priceString: '4.99', credits: 300 },
          { product_id: 'plan_standard', tier: 'standard', name: 'Standard', price: 9.99, priceString: '9.99', credits: 600 },
          { product_id: 'premium_plan', tier: 'premium', name: 'Premium', price: 19.99, priceString: '19.99', credits: 1200 },
          { product_id: 'extra_credits', tier: 'extra', name: 'Extra Credits', price: 29.99, priceString: '29.99', credits: 900 }
        ];
        setIapProducts(products as any);
      }
    } catch (error) {
      LoggerService.error('[PlanSelection] Error loading products:', error);
      const products = [
        { product_id: 'plan_basic', tier: 'basic', name: 'Basic', price: 4.99, priceString: '4.99', credits: 300 },
        { product_id: 'plan_standard', tier: 'standard', name: 'Standard', price: 9.99, priceString: '9.99', credits: 600 },
        { product_id: 'premium_plan', tier: 'premium', name: 'Premium', price: 19.99, priceString: '19.99', credits: 1200 },
            ...
            {currentTier === 'premium' && (
              ...
              >
                ...
                <div className="mb-3 mt-2">
                  ...
                  <div className="flex items-baseline gap-1">
                    <span className="text-2xl font-bold text-white">{extraCreditsProduct?.priceString || '29.99'}</span>
              ...
              {purchasing ? (
                ...
                <>
                  ...
                  {selectedTier === 'free' ? I18N.continue[language] :
                   isExtraCreditsSelected ? (language === 'es' ? `Comprar Créditos Extra - ${extraCreditsProduct?.priceString || '29.99'}` :
                                             language === 'pt' ? `Comprar Créditos Extras - ${extraCreditsProduct?.priceString || '29.99'}` :
                                             language === 'it' ? `Acquista Crediti Extra - ${extraCreditsProduct?.priceString || '29.99'}` :
                                             language === 'de' ? `Extra Credits kaufen - ${extraCreditsProduct?.priceString || '29.99'}` :
                                             language === 'fr' ? `Acheter Crédits Supplémentaires - ${extraCreditsProduct?.priceString || '29.99'}` :
                                             `Buy Extra Credits - ${extraCreditsProduct?.priceString || '29.99'}`) :
                   (() => {
                     ...
                     const buyText = language === 'es' ? 'Comprar Plan' :
                                   ...
                                   'Buy Plan';
                     return `${buyText} ${planName} - ${product.priceString}`;
                   })()
                  }
                </>
              )}
